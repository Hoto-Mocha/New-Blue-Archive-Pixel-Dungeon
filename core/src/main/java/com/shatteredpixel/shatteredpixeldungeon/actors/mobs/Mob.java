...
public abstract class Mob extends Char {
	...
	public int defenseProc( Char enemy, int damage ) {
		...
		if (Dungeon.isChallenged(Challenges.ANGRY_ENEMIES) && state == SLEEPING && buff(MagicalSleep.class) == null) {
...
//			Badges.validateRogueUnlock();
	...
	public void destroy() {
		...
		if (Dungeon.hero.isAlive()) {
			...
			if (alignment == Alignment.ENEMY) {
				...
				if (Dungeon.hero.buff(QuestEnemy.QuestEnemyTracker.class) != null) {
					exp *= 2;
		...
		if (alignment == Alignment.ENEMY){
			...
			if (cause == Dungeon.hero && Dungeon.level.heroFOV[pos]) {
				if (Random.Int(4) == 0) { //25% chance
					Dungeon.hero.yellI("enemy_defeated_" + (1 + Random.Int(5))); //1~5 variable
					GLog.newLine();
				}
			}
		}
	...
	public float lootChance(){
		...
		dropBonus += YuzuStatus.yuzuDropMulti(Dungeon.hero)-1f;
