...
public enum Talent {
	...
	//Ratmogrify T4
	RATSISTANCE(23, 9, 4), RATLOMACY(24, 9, 4), RATFORCEMENTS(25, 9, 4);
	...
	public static class InstantFocusingTracker extends FlavourBuff{
		{ type = buffType.POSITIVE; }
		public int icon() { return BuffIndicator.INVERT_MARK; }
		public void tintIcon(Image icon) { icon.hardlight(1f, 1f, 0.0f); }
		public float iconFadePercent() { return Math.max(0, 1f - (visualcooldown() / 5)); }
	...
	public static class TacticalSuppressTracker extends Buff {}
...
//					break;
	...
	public static void onTalentUpgraded( Hero hero, Talent talent ){
		...
		if (talent == HEIGHTENED_SENSES || talent == FARSIGHT || talent == DIVINE_SENSE || talent == MIYAKO_T2_3 || talent == MIYAKO_EX2_3 || talent == HOSHINO_T2_3 || talent == SHIROKO_T2_3){
			Dungeon.observe();
		}
		...
		if (talent == SHIROKO_T1_2 && !ShardOfOblivion.passiveIDDisabled()) {
			if (hero.pointsInTalent(SHIROKO_T1_2) == 1) {
				if (hero.belongings.weapon() instanceof AR)  {
					hero.belongings.weapon.identify();
				}
			}
			if (hero.pointsInTalent(SHIROKO_T1_2) == 2) {
				for (Item i : hero.belongings.getAllItems(AR.class)) {
					i.identify();
				}
			}
		}

		if (talent == NOA_T1_2 && !ShardOfOblivion.passiveIDDisabled()) {
			if (hero.pointsInTalent(NOA_T1_2) == 1) {
				if (hero.belongings.weapon() instanceof HG)  {
					hero.belongings.weapon.identify();
				}
			}
			if (hero.pointsInTalent(NOA_T1_2) == 2) {
				for (Item i : hero.belongings.getAllItems(HG.class)) {
					i.identify();
				}
			}
		}
	...
	public static void onFoodEaten( Hero hero, float foodVal, Item foodSource ){
		...
		if (hero.hasTalent(Talent.HOSHINO_T2_1) && hero.buff(Drowsy.class) == null) {
			Buff.affect(hero, Drowsy.class, 275-75*hero.pointsInTalent(Talent.HOSHINO_T2_1));
		}
	...
	public static void onItemCollected( Hero hero, Item item ){
		...
		if (hero.pointsInTalent(SHIROKO_T1_2) == 2 && (item instanceof AR)){
			identify = true;
		}

		if (hero.pointsInTalent(NOA_T1_2) == 2 && (item instanceof HG)){
			identify = true;
		}
